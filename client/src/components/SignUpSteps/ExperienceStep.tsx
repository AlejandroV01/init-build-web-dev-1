...
const ExperienceStep: React.FC<Props> = ({ handleStepSubmit }) => {
  const user = useAppSelector(state => state.auth)
  const [experiences, setExperiences] = useState<Experience[]>([experienceObject])
  ...
  const handleSubmit = () => {
    let check = false
    experiences.forEach(eduction => {
      if (
        eduction.company === '' ||
        eduction.position === '' ||
        eduction.startMonth === '' ||
        eduction.startYear === '' ||
        (!eduction.isPresent && (eduction.endMonth === '' || eduction.endYear === ''))
      ) {
  ...
  return (
    <CardBox>
      ...
      >
        ...
        <span className='text-sm text-slate-500'>List your experience(s) here!</span>
                  ...
                  <div className='col-span-6'>
                    <div className='grid grid-cols-6 gap-3'>
                      ...
                      <div className='col-span-6 flex flex-row align-middle gap-1 checkbox'>
                        <CheckboxComponent checked={experience.isPresent} onChange={() => handleChecked(i)} />
